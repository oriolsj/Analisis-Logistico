...
#sapply(data[, medicamentos], unclass)
```
...
for (j in medicamentos) {  ##Cada vez que encuentra una celda vacia problemas
  if( sum(is.na(data[[j]])) == length(data[[j]]) ){ #Si es nula la obviamos del analisis
    ...
    print(j)
    data[[j]] <- lapply( data[[j]], function(cell){
    if (is.null(cell) | is.na(cell) | is.nan(cell)) {
      # Si la celda es NULL, NA or NAN return devolvemos la celda sin cambiar el valor
      return(cell);
    } else {
      #  Cambiamos las categorias a numeros
        return( recode(cell,
                    "Sensible" = 0,
                    "Sensible EI" = 1,
                    "Resistente" = 2))
